// =====================================================
//  Plantilla Elegante · 15 años · script.js
// =====================================================
...
function init(c) {
  ...
  initSparkles('cover-sparkles');
  initSparkles('hero-stars');
  initCover();
  initCopy();
  initLightbox();
  initMusica(c.musica);
...
function aplicarTema(tema) {
  ...
  if (tema.acento) {
    ...
    r.setProperty('--silver-glow', `rgba(${rv},${g},${b},.18)`);
  }
...
function initSparkles(containerId) {
  ...
  const count = containerId === 'cover-sparkles' ? 65 : 42;
  for (let i = 0; i < count; i++) {
    const el = document.createElement('div');
    const isBurst = Math.random() < 0.2; // 20% con destello esporádico
    el.className = 'sparkle' + (isBurst ? ' sparkle--burst' : '');
    const size = isBurst ? 1.5 + Math.random() * 2 : .7 + Math.random() * 2.2;
    el.style.cssText = `
      ...
      --op:    ${isBurst ? .8 + Math.random() * .2 : .2 + Math.random() * .55};
    `;
